...
- Creates five badge layouts: a blue/cyan 16-bit NightCode ID, an Allied Universal staff card, a generic **DEMO STATE / SPECIMEN**, and New York specimen designs in **16-bit** and **polished** styles. The staff layout takes a cardholder photo, name, title, height, weight, eye color, date of hire, department, site, and employee ID. It saves local records and exports 300 DPI CR80-sized PNG or PDF files; no badge printer is required.
- New York designs are editable in **Badge Studio**. Select either New York template, enter a name, specimen reference, role/class, fictional location, collection, designer, dates and portrait, then switch **View** between Front and Back. **Export PNG / PDF** saves the selected side; **Export front + back PDF** saves two pages. Both sides share one saved record. Pixel and grayscale photo treatments match the two styles. Permanent specimen markings and DEMO-NY references identify these as design studies, not state-issued identification.
...
Run `py -3 -m unittest test_badge_studio -v` from this directory to verify saved-record loading, portrait updates, existing templates, validation and front/back PDF exports.
