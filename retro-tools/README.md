...
The mascots, Genesis-era character portraits, Disk Dude branding, and disc/badge/folder/lock/game icons are SpriteCook exports in `assets/spritecook/`. The NightCode badge plate in `assets/nightcode-badge-gpt-v2.png` is GPT-generated 16-bit artwork; Netcon layers editable text and an optional photo over it. The Netcon emblem and wide marquee are the user-supplied images in `assets/user-provided/`; they drive Netcon's window icon and live header. `make_assets.py` installs the approved SpriteCook and supplied files into `assets/`, makes Windows icons, and updates `spritecook-assets.json` with source IDs and hashes. Running it again preserves the NightCode badge plate and supplied Netcon branding.
